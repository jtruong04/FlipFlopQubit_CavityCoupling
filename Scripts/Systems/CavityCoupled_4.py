...
class CavityCoupled_4:
    ...
    def getNoiseHamiltonian(self, approxNoise = None):
        ...
        if self.n0 == 0:
            ...
            hL = np.array([
                           [  w03+w30+w33-2*dzz*n0,           0, -gc*np.sqrt(n0)*wp/dc],
                           [                     0, w03+w30+w33,               -dxy*n0], 
                           [ -gc*np.sqrt(n0)*wp/dc,     -dxy*n0, -w03+w30-w33-2*dxy*n0]
                        ])
            hR = np.array([
                           [  w03+w30+w33-2*dzz*n0, -gc*np.sqrt(n0)*wp/dc,           0], 
                           [ -gc*np.sqrt(n0)*wp/dc, -w03+w30-w33-2*dxy*n0,     -dxy*n0], 
                           [                     0,               -dxy*n0, w03+w30+w33]
                        ])
        elif self.n0 == 2:
            hL = np.array([
                           [   w03+w30+w33-2*dzz*n0,                        0,          -gc*np.sqrt(n0)*wp/dc,                         0],
                           [                      0, w03+w30+w33-2*dzz*(n0-1),                            -dxy,  -gc*np.sqrt(n0-1)*wp/dc], 
                           [ -gc*np.sqrt(n0)*wp/dc,                  -dxy, -w03+w30-w33-2*dzz*(n0-1)-2*dxy,                         0],
                           [                      0, -gc*np.sqrt(n0-1)*wp/dc,                               0, -w03+w30-w33-2*dxy*(n0-1)]
                        ])
            hR = np.array([
                           [   w03+w30+w33-2*dzz*n0,          -gc*np.sqrt(n0)*wp/dc,                        0,                         0],
                           [ -gc*np.sqrt(n0)*wp/dc, -w03+w30-w33-2*dzz*(n0-1)-2*dxy,                  -dxy,                         0],
                           [                      0,                            -dxy, w03+w30+w33-2*dzz*(n0-1),  -gc*np.sqrt(n0-1)*wp/dc], 
                           [                      0,                               0, -gc*np.sqrt(n0-1)*wp/dc, -w03+w30-w33-2*dxy*(n0-1)]
                        ])
        else:
            hL = np.array([
                           [   w03+w30+w33-2*dzz*n0,                        0,          -gc*np.sqrt(n0)*wp/dc,                                      0],
                           [                      0, w03+w30+w33-2*dzz*(n0-1),                        -dxy,               -gc*np.sqrt(n0-1)*wp/dc], 
                           [ -gc*np.sqrt(n0)*wp/dc,                  -dxy, -w03+w30-w33-2*dzz*(n0-1)-2*dxy,                                      0],
                           [                      0, -gc*np.sqrt(n0-1)*wp/dc,                               0, -w03+w30-w33-2*dxy*(n0-1)-2*dzz*(n0-1)]
                        ])
            hR = np.array([
                           [   w03+w30+w33-2*dzz*n0,          -gc*np.sqrt(n0)*wp/dc,                        0,                                      0],
                           [ -gc*np.sqrt(n0)*wp/dc, -w03+w30-w33-2*dzz*(n0-1)-2*dxy,                  -dxy,                                      0],
                           [                      0,                        -dxy, w03+w30+w33-2*dzz*(n0-1),               -gc*np.sqrt(n0-1)*wp/dc], 
                           [                      0,                               0, -gc*np.sqrt(n0-1)*wp/dc, -w03+w30-w33-2*dxy*(n0-1)-2*dzz*(n0-1)]
                        ])
    ...
    def getSystemHamiltonian(self, approxInteraction = None):
        ...
        if self.n0 == 0:
            ...
            return np.array([[        -self.df-dd*self.n0, self.g_f * np.sqrt(self.n0), self.g_f * np.sqrt(self.n0)],
                             [self.g_f * np.sqrt(self.n0),                           0,                           0],
                             [self.g_f * np.sqrt(self.n0),                           0,                           0]
                            ])
        elif self.n0 == 2:
            return np.array([[                -self.df-dd,   self.g_f*np.sqrt(self.n0),   self.g_f*np.sqrt(self.n0),                          0],
                             [self.g_f * np.sqrt(self.n0),                           0,                           0,self.g_f*np.sqrt(self.n0-1)],
                             [self.g_f * np.sqrt(self.n0),                           0,                           0,self.g_f*np.sqrt(self.n0-1)],
                             [                          0, self.g_f*np.sqrt(self.n0-1), self.g_f*np.sqrt(self.n0-1),     self.df-dd*(1-self.n0)]
                             ])
        else:
            return np.array([[                 -self.df-dd,   self.g_f*np.sqrt(self.n0),   self.g_f*np.sqrt(self.n0),                           0],
                             [ self.g_f * np.sqrt(self.n0),                           0,                           0, self.g_f*np.sqrt(self.n0-1)],
                             [ self.g_f * np.sqrt(self.n0),                           0,                           0, self.g_f*np.sqrt(self.n0-1)],
                             [                          0 , self.g_f*np.sqrt(self.n0-1), self.g_f*np.sqrt(self.n0-1),                  self.df+dd]
                             ])        
        return 
