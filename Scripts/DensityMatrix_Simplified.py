...
def computeDensityMatrix(system, initialState, T, Nt):
    ...
    noise = [np.diag(np.matmul(np.matmul(states_inverse, hn), states))/hbar
             for hn in noise]
    gamma_squared = [((np.kron(hn, np.ones(num_states)) - np.kron(np.ones(num_states), hn))**2).reshape(num_states,num_states) for hn in noise]
    Ki = [-np.vectorize(param['type'].J)(timeRange)[:,np.newaxis,np.newaxis]*gsquared[np.newaxis,:] for gsquared, param in zip(gamma_squared, noiseParameters)]
    G = np.exp(sum(Ki))
